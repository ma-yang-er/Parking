...
public class tempOperation {
    ...
    public void deleteEntity(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        response.sendRedirect("/TempHandle?type=4");//删除成功后跳转至管理页面
    }
    ...
    public void updateEntity(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException
    {
        ...
        {
            try {
                response.sendRedirect("/TempHandle?type=4");//成功更新数据后跳转至TempMsg.jsp页面
            } catch (IOException e) {
        ...
        if(!temp.checkExist(card_id))
        {
            ...
            if((card_id!=null &&card_id!="")&&(car_num!=null&&car_num!="")){
                ...
                {	System.out.println("card_id="+card_id);
                    ...
                    out.write("<script>alert('数据添加成功！'); location.href = '/TempHandle?type=4';</script>");
                }
                else {
                    out.write("<script>alert('数据添失败！'); location.href = '/TempHandle?type=4';</script>");
                }
            }
            else{
                out.write("<script>alert('临时IC卡号或者车牌号不能为空'); location.href = '/TempHandle?type=4';</script>");
            }

        }
        else {
            out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/TempHandle?type=4';</script>");
        }
    }
