...
public class userOperation {
    ...
    public void deleteEntity(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        response.sendRedirect("/UserHandle?type=4");//删除成功后跳转至管理页面
    }
    ...
    public void insertEntity(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException, IOException
    {
        ...
        if(!user.checkExist(user_id))
        {
            ...
            {
                out.write("<script>alert('数据添加成功！'); location.href = '/UserHandle?type=4';</script>");
            }
            else {
                out.write("<script>alert('数据添失败！'); location.href = '/UserHandle?type=4';</script>");
            }
        }
        else {
            out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/UserHandle?type=4';</script>");
        }
    }
